...
var RecurringDates = function() {
	...
	*/
	self.init = function(_dateModel, _tzDesignation) {
		console.log('RecurringDates.init()')
	...
	*/
	...
	self.parseModel = function() {
		for (var i in self.dateModel) {
			console.log(' checking: ' + self.dateModel[i].name)
			...
			console.log('  - next show: ' + self.dateModel[i].nextStartDate, '\n')
		}
	}
	...
	self.getShowtimeDatesOfNext = function(_event) {
		...
		for (var i in _event.tuneins) {
			console.log(
				'  - checking tunein: ' +
					_event.tuneins[i].days +
					', startTime: ' +
					_event.tuneins[i].startTime +
					' - endTime: ' +
					_event.tuneins[i].endTime
			)
			for (var j in _event.tuneins[i].days) {
				var eventDayIndex = self.getDayIndexFor(_event.tuneins[i].days[j])
				var daysToNextShow = 0
				//console.log( 'checking event day-index: ' + eventDayIndex + ', now day-index: ' + dayIndexForNow );
				if (eventDayIndex >= dayIndexForNow) daysToNextShow = eventDayIndex - dayIndexForNow
				else if (eventDayIndex < dayIndexForNow) daysToNextShow = 7 - dayIndexForNow + eventDayIndex

				if (daysToNextShow < fewestDaysToNextShow) {
					var eventTimeDates = self.buildShowtimeDatesFor(_event.tuneins[i], daysToNextShow)
					//console.log( 'now: ' + DateUtils.getNow( this.tzDesignation ) + '\nstart: ' + eventTimeDates.startDate + '\nend: ' + eventTimeDates.endDate );
		...
		if (!nextShow) return lastShow
		else return nextShow
	}

	self.buildShowtimeDatesFor = function(_tunein, _daysToNextShow) {
		//console.log( 'RecurringDates.buildShowtimeDatesFor(), tunein: ' + _tunein + ', daysToNextShow: ' + _daysToNextShow );
		var result = {}
	...
	self.adjustDateToStartOfDay = function(_date) {
		...
		if (todayMinutes < DateUtils.newDayStartsAt) now.setHours(now.getHours() - 24)
		else now.setHours(0)
		now.setMinutes(0)
		return now
	}
	...
	self.getDayIndexForNow = function() {
		var now = DateUtils.getNow(self.tzDesignation)
		var todayMinutes = now.getHours() * 60 + now.getMinutes()
		if (todayMinutes < DateUtils.newDayStartsAt) return now.getDay() > 0 ? now.getDay() - 1 : 6
