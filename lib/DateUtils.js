...
var DateUtils = new function() {
	...
	*/
	self.setCurrentDate = function(_dateOrTimestamp, _tzDesignation) {
		...
		console.log('')
		console.log('')
		console.log('    DATE-MODE: ' + self.dateMode)
		...
		console.log(
			'      ' +
				self
					.getCurrentDate()
					.date.toString()
					.replace(/GMT(\+|\-)[0-9]{4}.*$/, self.getDstAccurateGmtString(self.getCurrentDate().date, self.getCurrentDate().tzDesignation))
		)
		console.log('')
		console.log('')
		console.log('-------------------------------------------------------------------------------------------------------------')
	...
	self.parseUnixTimestamp = function(_unixTimestamp) {
		...
		if (gmtOffset.toUpperCase() == '-LOCAL') tzDesignation = self.TZ_LOCAL
		else {
	...
	/**
		...
		@example
			...
			if( easternDate.getTime() > easternNowDate.getTime() ) {
				console.log( 'easternDate has not yet happened' );
			}
			else {
				console.log( 'easternDate is passed' );
			}
	*/
	...
	*/
	self.toSimpleTime = function(date) {
		var time =
			...
			':' +
			TextUtils.pad(date.getMinutes(), 2)
		return time
	}
	...
	*/
	...
	self.toDayAndMonth = function(date, abbreviate, returnMonthsAsNumbers, padMonth, useNumericSuffixes) {
		var _months = abbreviate ? self.getLabels().MONTHS_ABRV : self.getLabels().MONTHS_FULL
		if (returnMonthsAsNumbers) return date.getDate() + '-' + self.getMonthAsNumber(date, padMonth)
		else {
	...
	*/
	self.selectMessagingForDate = function(date, tzDesignation, abbreviateMonths, abbreviateDays, monthsAsNumbers, padMonth) {
		...
		switch (true) {
			...
			default:
				if (self.language == 'spanish') return self.toDayAndMonth(date, abbreviateMonths, monthsAsNumbers, padMonth)
	...
	*/
	self.isLiveNow = function(date, context) {
		console.log('checking isLiveNow')
	...
	*/
	...
	self.isTonight = function(date, context) {
		...
		return (
			date.getTime() - context.getTime() < self.MS_PER_DAY &&
	...
	*/
	self.isTomorrow = function(date, context) {
		...
		return (
			context.getTime() >= _dayBeforeStart.getTime() + self.newDayStartsAt * 60 * 1000 &&
			context.getTime() < _dateStart.getTime() + self.newDayStartsAt * 60 * 1000
		)
	}
	...
	*/
	self.isThisWeek = function(date, context) {
		return (
			date.getTime() - context.getTime() <= self.MS_PER_WEEK &&
			((date.getDate() - context.getDate() == 6 && context.getHours() >= self.newDayStartsAt / 60) ||
				date.getDate() - context.getDate() < 6)
		)
	}
